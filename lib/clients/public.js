...
class PublicClient {
  ...
  makeRequestCallback(callback, resolve, reject, ignoreErrorCodes) {
    return (err, response, data) => {
      ...
      if (err) {
        ...
        err.data = data;
                 ...
                 !ignoreErrorCodes.includes(response.statusCode)) {
        err = new Error(
  ...
  request(method, uriParts, opts = {}, ignoreErrorCodes = [], callback) {
  ...
  getProductTrades(productID, args, ignoreErrorCodes, callback) {
    [productID, args, callback] = this._normalizeProductArgs(
      productID,
      args,
      callback,
    ...
    return this.get(path, { qs: args }, ignoreErrorCodes, callback);
  }
  ...
  getProductTradeStream(productID, tradesFrom, tradesTo) {
    ...
    function fetchTrades(stream, tradesFrom, tradesTo, shouldStop) {
      ...
      this.getProductTrades(productID, opts, [429], (err, resp, data) => {
        if (err) {
